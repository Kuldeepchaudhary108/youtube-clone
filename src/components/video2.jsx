...
export default function Video() {
  ...
  const fetchChannelData = async () => {
    ...
    await fetch(commentData_url)
      .then((res) => res.json())
      .then((data) => setCommentSection(data.items));
  };
  const fetchRelatedVideo = async () => {
    const related_url = `https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&chart=mostPopular&regionCode=IN&videoCategoryId=${categoryId}&key=${API_KEY}`;
